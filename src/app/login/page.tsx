...
export default function LoginPage() {
  ...
  return (
    <div className='flex justify-center items-center h-screen'>
      ...
      <h1 className='text-5xl font-mono mb-16 text-emerald-500 font-bold'>Log In</h1>
      <div>
      <span className='block text-xl text-emerald-50 font-mono font-bold mb-2'>Email</span>
      <input className='p-2 rounded-md bg-inherit w-96 text-fuchsia-400 font-semibold border-2 border-teal-100/70 focus:border-teal-100 focus:outline-none' type="text" placeholder='methamphetamine@gmail.com' value={user.email}
      onChange={(e)=>{
      ...
      <button className='mb-4 transition-all flex mt-6 items-center justify-center
       ...
       onClick={onLogin}
       >Log In</button>
      <span className='inline-flex text-lg text-center font-normal'> New User ? Go to <Link className='ml-2 underline font-semibold hover:text-emerald-400' href={'/signUp'}>Sign Up</Link></span>
      </div>
    </div>
  )
}
